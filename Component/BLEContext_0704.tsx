...
type BLEAction =
  ...
  | {type: 'PROCESS_BUFFER'; payload: number};
...
const bleReducer = (state: BLEState, action: BLEAction): BLEState => {
  switch (action.type) {
    ...
    case 'PROCESS_BUFFER':
      const newBuffer = [...state.bufferData, action.payload];
      if (newBuffer.length >= 50) {
        const updatedChartData = [...state.chartData, ...newBuffer];
        if (updatedChartData.length > 500) {
          updatedChartData.splice(0, updatedChartData.length - 500); // 오래된 데이터 제거
        }
        return {
          ...state,
          chartData: [...state.chartData, ...newBuffer], // 버퍼를 차트로 이동
          bufferData: [], // 버퍼 초기화
        };
      }
      return {
