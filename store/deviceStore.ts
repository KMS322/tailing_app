...
export const deviceStore = create<DeviceStore>((set, get) => ({
  ...
  checkCode: async (deviceCode: string) => {
    try {
      ...
      const response = await axios.post(`${API_URL}/device/check`, {
        deviceCode,
      });
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        set({
          checkIDSuccess: false,
          checkIDLoading: false,
          checkIDError: response.data.message,
        });
      }
    } catch (error: any) {
      set({
  ...
  findID: async (deviceCode: string) => {
    try {
      ...
      const response = await axios.post(`${API_URL}/user/find/id`, {
        deviceCode,
      });
      if (response.status === 200) {
        set({
          findIDSuccess: true,
          findIDLoading: false,
          findIDError: response.data.org_id,
        });
      }
    } catch (error: any) {
      set({
